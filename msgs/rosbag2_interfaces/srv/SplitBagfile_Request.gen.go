...
func NewSplitBagfile_RequestPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*SplitBagfile_RequestPublisher, error) {
	pub, err := node.NewPublisher(topic_name, SplitBagfile_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// SplitBagfile_Request
func NewSplitBagfile_RequestSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback SplitBagfile_RequestSubscriptionCallback) (*SplitBagfile_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SplitBagfile_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, SplitBagfile_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSplitBagfile_Request__Sequence = C.rosbag2_interfaces__srv__SplitBagfile_Request__Sequence

func SplitBagfile_Request__Sequence_to_Go(goSlice *[]SplitBagfile_Request, cSlice CSplitBagfile_Request__Sequence) {
	if cSlice.size == 0 {
		return
	}
...
func SplitBagfile_Request__Sequence_to_C(cSlice *CSplitBagfile_Request__Sequence, goSlice []SplitBagfile_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func SplitBagfile_Request__Array_to_Go(goSlice []SplitBagfile_Request, cSlice []CSplitBagfile_Request) {
	for i := 0; i < len(cSlice); i++ {
		SplitBagfile_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}
func SplitBagfile_Request__Array_to_C(cSlice []CSplitBagfile_Request, goSlice []SplitBagfile_Request) {
