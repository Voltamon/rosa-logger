...
// Stop_Response
func NewStop_ResponsePublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*Stop_ResponsePublisher, error) {
	pub, err := node.NewPublisher(topic_name, Stop_ResponseTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Stop_Response
func NewStop_ResponseSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback Stop_ResponseSubscriptionCallback) (*Stop_ResponseSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Stop_Response
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, Stop_ResponseTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CStop_Response__Sequence = C.rosbag2_interfaces__srv__Stop_Response__Sequence

func Stop_Response__Sequence_to_Go(goSlice *[]Stop_Response, cSlice CStop_Response__Sequence) {
	if cSlice.size == 0 {
		return
	}
...
func Stop_Response__Sequence_to_C(cSlice *CStop_Response__Sequence, goSlice []Stop_Response) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Stop_Response__Array_to_Go(goSlice []Stop_Response, cSlice []CStop_Response) {
	for i := 0; i < len(cSlice); i++ {
		Stop_ResponseTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}
func Stop_Response__Array_to_C(cSlice []CStop_Response, goSlice []Stop_Response) {
