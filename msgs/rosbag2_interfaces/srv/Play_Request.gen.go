...
// Play_Request
func NewPlay_RequestPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*Play_RequestPublisher, error) {
	pub, err := node.NewPublisher(topic_name, Play_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Play_Request
func NewPlay_RequestSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback Play_RequestSubscriptionCallback) (*Play_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Play_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, Play_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPlay_Request__Sequence = C.rosbag2_interfaces__srv__Play_Request__Sequence

func Play_Request__Sequence_to_Go(goSlice *[]Play_Request, cSlice CPlay_Request__Sequence) {
	if cSlice.size == 0 {
		return
	}
...
func Play_Request__Sequence_to_C(cSlice *CPlay_Request__Sequence, goSlice []Play_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Play_Request__Array_to_Go(goSlice []Play_Request, cSlice []CPlay_Request) {
	for i := 0; i < len(cSlice); i++ {
		Play_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}
func Play_Request__Array_to_C(cSlice []CPlay_Request, goSlice []Play_Request) {
